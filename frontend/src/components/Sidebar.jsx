import { Link, NavLink } from "react-router-dom"; // or whatever routing you use
...
const Sidebar = ({ isExpanded, setIsExpanded }) => {
  const links = [
    ...
    },
    {
      ...
      label: "Withdraw",
      icon: (
        <svg viewBox="0 0 512 512" className="size-5" fill="currentColor">
  ...
  return (
    <aside
      className={`fixed top-0 left-0 z-50 h-[100%] ${
        isExpanded ? "w-[280px]" : "w-13 sm:w-20"
      } bg-white shadow-lg lg:relative transform transition-all duration-300 ease-in-out`}
    >
      <div className="flex justify-between items-center py-6 px-4 sm:px-6 h-[80px] border-b border-gray-300 bg-white z-10">
        <AnimatePresence>
          {isExpanded && (
            <motion.div
              key="Logo"
        ...
        >
          {!isExpanded ? (
            <FaBars className="size-5 sm:size-6" fill="currentColor" />
          ) : (
            <FaTimes className="size-5 sm:size-6" fill="currentColor" />
          )}
        </button>
      </div>

      <nav className="py-4 px-1.5 sm:px-4 h-[100%] pb-[90px]">
        <div className="flex flex-col justify-between w-full h-full">
          <div>
            {links.map((link, i) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center p-2 sm:p-3 rounded-lg mb-2 h-[36px] sm:h-[40px] gap-4 transition-colors ${
          ...
          <div
            ...
            className="cursor-pointer h-[36px] sm:h-[40px] flex gap-2 items-center p-2 sm:p-3 rounded-lg bg-gray-200 text-gray-700 hover:bg-[#a06eff] hover:text-white"
          >
            {(isLoading && (
